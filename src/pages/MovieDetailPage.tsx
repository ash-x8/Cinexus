  ...
  return (
    <div className="space-y-10 pb-16">
      ...
      <div>
        <Link
          to="/"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#12151e] border border-white/10 text-gray-300 hover:text-white hover:border-cyan-400 text-xs font-bold transition-all shadow-md"
        >
      ...
      <div className="relative rounded-3xl overflow-hidden glass-panel border border-white/10 shadow-2xl">
        <div className="relative min-h-[300px] md:min-h-[380px] w-full flex items-end">
          <img
          ...
          <div className="relative z-10 p-6 sm:p-10 w-full flex flex-col md:flex-row items-start md:items-end gap-6">
            <img
              src={movie.posterUrl}
              alt={movie.title}
              className="w-32 sm:w-44 aspect-[2/3] object-cover rounded-2xl border-2 border-white/20 shadow-2xl flex-shrink-0"
            />

            <div className="space-y-3 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-3 py-1 rounded-lg bg-cyan-400 text-black font-extrabold text-xs uppercase tracking-wider">
                  {movie.qualityBadge}
                </span>
                {movie.hasSinhalaSub && (
                  <span className="px-3 py-1 rounded-lg bg-purple-600 border border-purple-400/40 text-white font-bold text-xs flex items-center gap-1">
                    <Subtitles className="w-3.5 h-3.5 text-cyan-300" />
                    Sinhala Subbed (සිංහල උපසිරැසි)
                  </span>
                )}
                {movie.isDualAudio && (
                  <span className="px-3 py-1 rounded-lg bg-indigo-600/80 text-cyan-200 font-bold text-xs">
                    Dual Audio
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-gray-300 pt-1">
                <span className="flex items-center gap-1 text-amber-400 bg-black/60 px-2.5 py-1 rounded-lg border border-amber-500/30">
      ...
      {/* Embedded Multi-Server Video Player Section */}
      <section className="glass-panel p-6 rounded-3xl border border-white/10 shadow-2xl space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
          <div>
            <h2 className="text-lg font-black text-white flex items-center gap-2">
              <Play className="w-5 h-5 text-cyan-400 fill-cyan-400" />
              Online Movie Streaming Player (නරඹන්න)
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">Switch servers if a player encounters loading delays.</p>
          </div>
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5 ${
                  activeServer === server.id
                    ...
                    : 'bg-[#12151e] text-gray-400 hover:text-white border border-white/10'
                }`}
              >
      ...
      <section className="glass-panel p-6 rounded-3xl border border-white/10 shadow-2xl space-y-6">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div>
            <h2 className="text-lg font-black text-white flex items-center gap-2">
              ...
              Multi-Quality Direct Downloads (බාගත කරගන්න)
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">High-speed links with Sinhala subtitle embedded</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {movie.downloadLinks?.map((link) => (
            <div
              key={link.id}
              className="glass-card p-4 rounded-2xl flex flex-col justify-between space-y-3 border border-white/10 hover:border-cyan-400/50 transition-all"
            >
              ...
              <div>
                <p className="text-xs font-semibold text-white">{link.format}</p>
                <p className="text-[11px] text-gray-400">Direct High Speed Server</p>
              </div>
              <button
                onClick={() => handleDownloadClick(link.quality, link.url)}
                className="w-full py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-md transition-all"
              >
                {link.quality === 'Telegram' ? (
                  <>
                    <Send className="w-3.5 h-3.5 text-cyan-300" /> Telegram Link
                  </>
                ) : (
                  <>
      ...
      {/* Dual Language Plot & Subtitle Meta */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

        {/* Left Column: Plot Summary (Sinhala & English Tabs) */}
        <div className="lg:col-span-2 glass-panel p-6 rounded-3xl border border-white/10 shadow-2xl space-y-5">
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            ...
            <div className="flex items-center bg-[#12151e] p-1 rounded-xl border border-white/10">
              <button
          ...
          <div className="pt-4 border-t border-white/5 grid grid-cols-2 gap-4 text-xs">
            <div>
              <span className="text-gray-400 block mb-1">Cast Members:</span>
        ...
        <div className="glass-panel p-6 rounded-3xl border border-white/10 shadow-2xl space-y-4 flex flex-col justify-between">
          <div>
            ...
            <div className="mt-4 flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-600 to-cyan-500 p-[2px]">
                <div className="w-full h-full bg-[#12151e] rounded-[14px] flex items-center justify-center font-bold text-cyan-400 text-lg">
                  {movie.subtitleAuthor?.name?.[0] || 'C'}
                </div>
              </div>
              <div>
                <h4 className="text-sm font-bold text-white">{movie.subtitleAuthor?.name || 'CINEXUS Official Subber'}</h4>
                <p className="text-xs text-purple-300 font-medium">Senior Sinhala Subtitle Creator</p>
              </div>
            </div>

            <div className="mt-5 space-y-2 text-xs bg-[#12151e] p-3.5 rounded-2xl border border-white/5">
              <div className="flex justify-between text-gray-400">
                <span>Total Downloads:</span>
                <span className="font-bold text-cyan-400">{movie.subtitleAuthor?.downloadsCount?.toLocaleString() || '12,500'}</span>
              </div>
              <div className="flex justify-between text-gray-400">
                <span>Release Date:</span>
                <span className="font-bold text-white">{movie.subtitleAuthor?.releaseDate || 'Recently Added'}</span>
              </div>
              <div className="flex justify-between text-gray-400">
                <span>Audio Language:</span>
                <span className="font-bold text-white">{movie.audioLanguage}</span>
              </div>
            </div>
          </div>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-full py-3 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black font-extrabold text-xs flex items-center justify-center gap-2 shadow-lg transition-all"
          >
