...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 glass-nav transition-all duration-300 border-b border-white/10">
      {/* Dynamic Top Announcement Banner & Live Viewers counter */}
      {siteSettings.showAnnouncement && siteSettings.announcementText && (
        <div className="bg-gradient-to-r from-purple-950 via-rose-950 to-indigo-950 border-b border-rose-500/20 px-4 py-1.5 text-center text-[11px] font-bold text-amber-300 flex flex-wrap items-center justify-between gap-2 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-full text-emerald-400 text-[10px] font-extrabold whitespace-nowrap">
            ...
            <span>🟢 Live Viewers Streaming Now: {(analytics?.activeStreams || 1420).toLocaleString()}</span>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          {/* Logo & Brand (No admin icon / shield visible) */}
          <Link to="/" className="flex items-center gap-3 group">
            <div className="relative flex items-center justify-center w-11 h-11 rounded-2xl bg-gradient-to-br from-purple-600 via-rose-600 to-amber-400 p-[2px] shadow-lg shadow-purple-900/40 group-hover:shadow-amber-500/30 transition-all duration-300">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-2xl font-black tracking-tight bg-gradient-to-r from-white via-slate-100 to-amber-300 bg-clip-text text-transparent">
                  {siteSettings.siteTitle || 'CINEXUS'}
                </span>
                <span className="px-1.5 py-0.5 text-[9px] font-black rounded bg-amber-500/20 text-amber-300 border border-amber-500/30 uppercase tracking-widest">
                  SINHALASUB
                </span>
              </div>
              <p className="text-[11px] font-semibold text-purple-300/90 tracking-wider flex items-center gap-1">
                {siteSettings.sinhalaTitle || 'සිනෙක්ස්'} <span className="inline-block w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> Sinhala Subtitled HD
              </p>
            </div>
          </Link>
          ...
          <div className="hidden md:block relative flex-1 max-w-md mx-8" ref={searchRef}>
            <div className={`relative flex items-center rounded-2xl bg-[#121620]/90 border transition-all duration-300 ${
              ...
              <Search className={`w-4 h-4 ml-4 transition-colors ${isSearchFocused ? 'text-cyan-400' : 'text-gray-400'}`} />
              <input
                type="text"
                ...
                className="w-full py-2.5 px-3 bg-transparent text-xs text-white placeholder-gray-400 focus:outline-none"
              />
              {searchQuery && (
                <button
            ...
            {isSearchFocused && searchSuggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#121620]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="px-4 py-2 border-b border-white/10 flex justify-between items-center bg-purple-950/30">
                  <span className="text-xs font-bold text-purple-300 flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-cyan-400" /> Instant Search Results
                  </span>
                  ...
                  {searchSuggestions.map((movie) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="text-xs font-extrabold text-white group-hover:text-cyan-400 truncate mt-1 transition-colors">
                          {movie.title}
                        </h4>
                        <p className="text-[11px] text-purple-200/80 truncate leading-snug">{movie.sinhalaTitle}</p>
                      </div>
          ...
          <div className="hidden lg:flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-2 text-xs font-extrabold rounded-xl bg-purple-950/60 border border-purple-500/30 text-purple-300 hover:text-white hover:bg-purple-900/80 transition-all shadow-sm"
            >
              Sinhala Subbed
            </button>
            <button
              onClick={() => handleCategorySelect('TV Series')}
              className="px-3.5 py-2 text-xs font-extrabold rounded-xl bg-cyan-950/60 border border-cyan-500/30 text-cyan-300 hover:text-white hover:bg-cyan-900/80 transition-all shadow-sm"
            >
              TV Series
            </button>
          </div>
        ...
        <div className="md:hidden pb-4">
          <div className="relative flex items-center rounded-2xl bg-[#121620] border border-white/10">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#0a0b0e] border-b border-white/10 px-4 py-4 space-y-4 animate-in slide-in-from-top duration-300">
          <div>
